...
@section('service_locations_active', 'active')
...
<div>
    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <div class="d-flex align-items-center justify-content-between">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Admin /</span> Service Location Management
                </h4>

                <div class="d-flex align-items-center">
                    <button data-bs-toggle="modal" data-bs-target="#addServiceLocation" class="btn btn-primary">
                        <span class="tf-icons bx bx-plus"></span>&nbsp; Add Service Location
                    </button>
                </div>
            </div>
                    ...
                    <table class="table">
                        ...
                        <tbody class="table-border-bottom-0">
                            @if (count($serviceLocations) > 0)
                                @foreach ($serviceLocations as $index => $serviceLocation)
                                    <tr>
                                        <td>{{ ($serviceLocations->currentPage() - 1) * $serviceLocations->perPage() + $index + 1 }}
                                        </td>
                                        <td>{{ $serviceLocation->name }}</td>
                                        <td>{{ $serviceLocation->country->name }}</td>
                                        <td>
                                            @if ($serviceLocation->active == true)
                                                <button
                                                    wire:confirm="Are you sure you want to change the status of this service location?"
                                                    wire:click.prevent="changeStatus({{ $serviceLocation->id }},0)"
                                            ...
                                            @elseif($serviceLocation->active == false)
                                                <button
                                                    wire:confirm="Are you sure you want to change the status of this service location?"
                                                    wire:click.prevent="changeStatus({{ $serviceLocation->id }},1)"
                                                    data-bs-toggle="tooltip" data-bs-offset="0,4"
                                                    data-bs-placement="top" data-bs-html="true" title=""
                                                    data-bs-original-title="<i class='bx bx-edit-alt bx-xs' ></i> <span>Tap To Change Status</span>"
                                            ...
                                            <div class="dropdown dropup">
                                                ...
                                                <div class="dropdown-menu" data-popper-placement="top-start">
                                                    <button class="dropdown-item"
                                                        wire:click="editServiceLocation({{ $serviceLocation->id }})">
                                                        <i class="bx bx-pencil me-1"></i> Edit
                                                    </button>
                                                    <button class="dropdown-item"
                                                        wire:click="deleteServiceLocation({{ $serviceLocation->id }})">
                ...
                <div class="container mt-4">
                    <div class="row">
                        <div class="col-12 d-flex align-items-center justify-content-end">
                            {{ $serviceLocations->links('custom-pagination') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div wire:ignore.self class="modal fade" id="addServiceLocation" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addServiceLocationTitle">Add New Service Location</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form>
                        <div class="mb-3">
                            <label class="form-label" for="serviceLocationName">Name</label>
                            <div class="input-group input-group-merge">
                        ...
                        <div class="mb-3">
                            <label class="form-label" for="countryDropdown">Timezone</label>
    ...
    <div wire:ignore.self class="modal fade" id="editServiceLocation" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editServiceLocationTitle">Edit Service Location</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form>
                        <div class="mb-3">
                            <label class="form-label" for="editServiceLocationName">Name</label>
                            <div class="input-group input-group-merge">
                                <span class="input-group-text"><i class="bx bx-heading"></i></span>
                        ...
                        <div class="mb-3">
                            <label class="form-label" for="editCountryDropdown">Country</label>
                            <select wire:model="editCountryId" class="form-control">
                                <option value="">Select Country</option>
                                @foreach ($countries as $country)
                                    <option value="{{ $country->id }}">{{ $country->name }}</option>
                                @endforeach
                        ...
                        <div class="mb-3">
                            <label class="form-label" for="countryDropdown">Timezone</label>
                            <select wire:model="editTimezone" class="form-control">
                                <option value="">Select Timezone</option>
                                @foreach ($timezones as $index => $timezone)
                                    <option value="{{ $index }}">{{ $timezone }}</option>
                                @endforeach
                        ...
                        <div class="mb-3">
                            <div class="form-check form-switch mb-3">
                                <input wire:model="editActive" class="form-check-input" type="checkbox"
                                    id="editActive">
    ...
    @script
        <script>
            $wire.on('hideAddServiceLocation', () => {
                $('#addServiceLocation').modal('hide');
            });
            $wire.on('editServiceLocation', () => {
                $('#editServiceLocation').modal('show');
            });
            $wire.on('hideEditServiceLocation', () => {
                $('#editServiceLocation').modal('hide');
            });
        </script>
    @endscript
