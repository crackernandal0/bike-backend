...
class ApprovedDrivers extends Component
{
    ...
    public function changeStatus($id, $status)
    {
        ...
        if ($user) {
            ...
            session()->flash('success', 'Driver account status updated successfully!');
        }
    }
    ...
    {
        $this->driverId = $id;
        $driver = Driver::with(['additionalInfo', 'bankInfo', 'documents'])->find($id);
    ...
    public function render()
    {
        ...
        $driver = Driver::query()->where('status', 'approved')->where('role', '!=', 'instructor');

        if ($this->search) {
            // Group the orWhere conditions together
        ...
        return view('livewire.driver.approved-drivers', compact('drivers', 'vehicleTypes', 'vehicleSubcategories'));
    }
}
